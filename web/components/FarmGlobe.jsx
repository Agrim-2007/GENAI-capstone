...
export default function FarmGlobe() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const wireGeometry = new THREE.SphereGeometry(1.765, 48, 48);
      ...
      resources.push(bandMaterial);

      for (let index = 0; index < 8; index += 1) {
        const bandGeometry = new THREE.TorusGeometry(1.78 + index * 0.006, 0.006, 8, 160);
        resources.push(bandGeometry);
      ...
      resources.push(fieldMaterial);

      for (let index = 0; index < 44; index += 1) {
        const lat = THREE.MathUtils.degToRad(-58 + Math.random() * 116);
        const lon = THREE.MathUtils.degToRad(Math.random() * 360);
        const radius = 1.785;
        const patchGeometry = new THREE.PlaneGeometry(0.16, 0.035);
        resources.push(patchGeometry);

        const patch = new THREE.Mesh(patchGeometry, fieldMaterial);
        patch.position.set(
      ...
      resources.push(particlesGeometry, particlesMaterial);
      ...
      const resize = () => {
        if (!mount || !renderer || disposed) return;
      ...
      console.warn('FarmGlobe fallback activated:', error);
  ...
  return (
    <div className="globe-shell">
      {hasWebGlError ? (
        <div className="globe-fallback" aria-label="Farm advisory globe fallback">
          Interactive globe unavailable in this browser context
        </div>
      ) : (
        <div
          ref={mountRef}
          className="h-[460px] w-full md:h-[560px]"
          aria-label="Rotating 3D farm globe"
        />
      )}
